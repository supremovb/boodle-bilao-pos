...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Calculate GROSS SALES (before discount), NET SALES (after discount), COGS, and DISCOUNTS
  let grossSales = 0;
  let totalDiscounts = 0;
  let totalCostOfGoodsSold = 0;

  allPaidNonVoided.forEach(p => {
    // Calculate subtotal (products + delivery) BEFORE discount
    let subtotal = 0;
    if (Array.isArray((p as any).products) && (p as any).products.length > 0) {
      subtotal = (p as any).products.reduce((s: number, prod: any) => s + (prod.price * prod.quantity), 0);
    } else {
      subtotal = typeof p.price === "number" ? p.price : 0;
    }
    
    // Add delivery charge to subtotal
    const deliveryCharge = typeof (p as any).deliveryCharge === "number" ? (p as any).deliveryCharge : 0;
    ...
    if ((p as any).discount && (p as any).discount > 0) {
      let discountAmount = 0;
      if ((p as any).discountType === 'percentage') {
        discountAmount = (subtotal * (p as any).discount) / 100;
      } else {
        discountAmount = (p as any).discount;
      }
    ...
    // Calculate COGS
    if (Array.isArray((p as any).products) && (p as any).products.length > 0) {
      totalCostOfGoodsSold += (p as any).products.reduce((s: number, prod: any) => {
  ...
  const netIncome = netSales - totalCostOfGoodsSold;
